...
- (void)viewDidLoad {
	...
	RACCommand *createAccountCommand = [[RACCommand alloc] initWithEnabled:formValid signalBlock:^RACSignal *(id input) {
		//create account is allowed, excuting this command
	...
	self.creatAccount.rac_command = createAccountCommand;
	UIColor *defaultButtonTitleColor = self.creatAccount.titleLabel.textColor;
	...
	[self.creatAccount rac_liftSelector:@selector(setTitleColor:forState:)
							withSignals:buttonTextColor, [RACSignal return:@(UIControlStateNormal)], nil];
	...
	//	derive the status labels text and color from our network result
